...
1. [Networks](../02-balve-infrastructure/01-networks.md)
2. [Servers & placement groups](../02-balve-infrastructure/02-servers-placement-groups.md)
3. [Firewalls](../02-balve-infrastructure/03-firewalls.md)
4. [SSH keys](../02-balve-infrastructure/04-ssh-keys.md)
5. [Server — master node](../02-balve-infrastructure/05-server-master-node.md)
6. [Server — worker node](../02-balve-infrastructure/06-server-worker-node.md)
7. [DNS records](../02-balve-infrastructure/07-dns-records.md) — make sure `balve.garmeres.com` and `*.balve.garmeres.com` point to the worker's public IP

Skip [Object storage](../02-balve-infrastructure/08-create-object-storage.md) — the S3 buckets already exist and contain the backups you'll restore from.
...
Follow [Deploy ArgoCD](../03-balve-deployment/01-deploy-argocd.md). This runs on the server via the Hetzner web console.
...
Follow [Sealed Secrets](../03-balve-deployment/02-create-sealed-secrets.md). This involves fetching credentials from Hetzner, Domeneshop, and GitHub, then running commands on your local machine.
...
Follow [Nextcloud OIDC for ArgoCD](../03-balve-deployment/03-nextcloud-oidc.md). This requires registering ArgoCD as an OAuth client in Nextcloud's admin panel.
